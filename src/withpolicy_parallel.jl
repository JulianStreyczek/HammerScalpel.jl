function withpolicy(policy::String, nprocs::Int64; TT=52, ndims=20, thetai=0.38, thetas=0.0044)
    ...
    # Store solver output
    rets = SharedArray{Int8,4}((TT,nSS,nII,nRR));
    # Dictionaries for translating output message <=> number
    ...
    # Empty matrices for results (shared because distributed programming)
    V0 = SharedArray{Float64,4}((TT,nSS,nII,nRR)); #value function
    qq = SharedArray{Float64,4}((TT,nSS,nII,nRR)); #mass tested/isolated
    ll = SharedArray{Float64,4}((TT,nSS,nII,nRR)); #mass of unknown types assigned to work sector
    mm = SharedArray{Float64,4}((TT,nSS,nII,nRR)); #mass of known types assigned to work sector
    ...
    for i in 1:TT
        ...
        @sync @distributed for j in 1:nSS
            for k in 1:nII
                for l in 1:nRR
                    S = Grid.S[j];
                    I = Grid.I[k];
    ...
    # Output information on solver success
    nsolve = sum(rets .!= 0);
    ...
    par = (gamma=Par.gamma, delta=Par.delta);
